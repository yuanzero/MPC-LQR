...
class iLQR():
    ...
    def forward_pass(self, X, U, k, K,npc_traj):
        ...
        for i in range(self.args.horizon):
            ...
            '''
            CBF_para_1, CBF_para_2 = self.constraints.obs_constraints[0].CBF_filter(X_new[:, i],npc_traj,i)
            if CBF_para_1[0,0]>0:
                lower_bound = CBF_para_2/CBF_para_1[0,0]
                U_new[0, i] = np.clip(U_new[0, i],lower_bound,None)
            elif CBF_para_1[0,0]<0:
                upper_bound = CBF_para_2/CBF_para_1[0,0]
                U_new[0, i] = np.clip(U_new[0, i],None,upper_bound)

            if CBF_para_1[0,1]>0:
                lower_bound = CBF_para_2/CBF_para_1[0,1]
                U_new[1, i] = np.clip(U_new[1, i],lower_bound,None)
            elif CBF_para_1[0,1]<0:
                upper_bound = CBF_para_2/CBF_para_1[0,1]
                U_new[1, i] = np.clip(U_new[1, i],None,upper_bound)
            '''
    ...
    def run_step(self, ego_state, npc_traj):
        ...
        X, U = self.get_optimal_control_seq(X_0, self.control_seq, poly_coeff, ref_traj[:, 0], npc_traj)  #iLQR 直接得到了path和control
        traj = X[:2, ::int(self.args.horizon/10)].T # traj is the planning path

        ## MPC替换
        initial_state = MPC.State(x=X_0[0], y=X_0[1], yaw=X_0[3], v=X_0[2])

        dl = 0.5  # course tick
        cx, cy, cyaw, ck = MPC.get_spine_course(traj, dl)

        sp = MPC.calc_speed_profile(cx, cy, cyaw, -180.0 / 3.6) #TARGET speed

        t, x, y, yaw, v, d, a = MPC.do_simulation(
            cx, cy, cyaw, ck, sp, dl, initial_state)

        Xstate = np.array([x, y, v, yaw])
        U = np.array([a,d])[:,1:]
        ####
    ...
    def get_optimal_control_seq(self, X_0, U, poly_coeff, x_local_plan, npc_traj):
        ...
        for itr in range(self.args.max_iters):
            ...
            X_new, U_new = self.forward_pass(X, U, k, K,npc_traj)
