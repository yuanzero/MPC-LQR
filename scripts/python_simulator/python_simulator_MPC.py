...
from ilqr.iLQR_MPC import iLQR
...
class PySimulator:
    ...
    def create_ilqr_agent(self):
        ...
        self.navigation_agent = iLQR(self.args, self.get_npc_bounding_box()) #替换MPC
    ...
    def run_step_ilqr(self):
        ...
        self.count += 1
        print("Controller: Acc {} Steer: {}".format(control[0, 1], control[1, 1]))

        return desired_path, local_plan, control[:,1]
    ...
    def animate(self, i):
        ...
        print("Controller: current_ego_state {} ".format(self.current_ego_state))
    ...
    def run_simulation(self):
        ...
        anim = animation.FuncAnimation(self.fig, self.animate,
                               ...
                               repeat=False)
        plt.show()
        #anim.save('BF_iLQR.gif',writer='imagemagick')
...
class SimParams:
    ...
    ## Car Parameters for LQR
    car_dims = np.array([4, 2])
    start_state = np.array([5, 0, 10, 0])  #INITIAL STATE [x, y, v, heading angle]
    max_speed = 180/3.6
    wheelbase = 2.94
    steer_min = -0.6
    steer_max = 0.6
